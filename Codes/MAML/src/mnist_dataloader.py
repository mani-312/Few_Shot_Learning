...
class MNIST_dataloader:
    ...
    def __init__(self, root, batchsz, n_way, k_shot, k_query, imgsz):
        ...
        self.resize = imgsz
        self.x = MNISTDataset(root='./data', train=True, transform=transform)

        temp = dict()  # {label:img1, img2..., 20 imgs, label2: img1, img2,... in total, 1623 label}
        for (img, label) in self.x:
            if label in temp.keys():
                temp[label].append(np.array(img))
                #try:
                ...
                #print(img.shape)
                temp[label] = [np.array(img)]
                #np.array(temp[label])

        self.x = []
        for label, imgs in temp.items():  
            print(np.array(imgs).shape)
            self.x.append(np.array(imgs[:5421])) # To make number of images per class same

            # as different class may have different number of imgs
        self.x = np.array(self.x).astype("float32")  # [[20 imgs],..., 1623 classes in total]
        ...
        self.indexes = {"test": 0}
        self.datasets = {"test": self.x}
        print("DB: test", self.x.shape)
        self.datasets_cache = {"test": self.load_data_cache(self.datasets["test"])}
    ...
    def load_data_cache(self, data_pack):
        ...
        for sample in range(10):  # num of episodes
            ...
            # [b, setsz, 1, 84, 84]
            x_spts = np.array(x_spts).astype("float32").reshape(self.batchsz, setsz, 1, self.resize, self.resize)
            y_spts = np.array(y_spts).astype("int").reshape(self.batchsz, setsz)
            # [b, qrysz, 1, 84, 84]
            x_qrys = np.array(x_qrys).astype("float32").reshape(self.batchsz, querysz, 1, self.resize, self.resize)
